  ...
  implicit val configReaderQueryConfig: ConfigReader[QueryConfig] =
    ConfigReader.fromCursor[QueryConfig] { cursor =>
      ...
      for {
        ...
        maxPendingRefreshNodeRequests <- objCur.getAtOpt[Int]("max-pending-refresh-node-requests")
          .map(_.getOrElse(defaultConfig.maxPendingRefreshNodeRequests))
        ...
        refreshNodeInterval <- objCur.getAtOpt[FiniteDuration]("refresh-node-interval")
          .map(_.getOrElse(defaultConfig.refreshNodeInterval))
