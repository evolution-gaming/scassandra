  ...
  implicit val configReaderSocketConfig: ConfigReader[SocketConfig] =
    ConfigReader.fromCursor[SocketConfig] { cursor =>
      ...
      for {
        ...
        connectTimeout <- objCur.getAtOpt[FiniteDuration]("connect-timeout")
          .map(_.getOrElse(defaultConfig.connectTimeout))
        readTimeout <- objCur.getAtOpt[FiniteDuration]("read-timeout")
          .map(_.getOrElse(defaultConfig.readTimeout))
        ...
        receiveBufferSize <- objCur.getAtOpt[Int]("receive-buffer-size")
          .map(_.orElse(defaultConfig.receiveBufferSize))
