 ...
 *
 * In other words it could be treated as set of [[DecodeByName]] and/or [[DecodeByIdx]]
 * instances, where the names or the indices of a required fields are already build-in.
 *
 * Example:
 * {{{
...
object DecodeRow {
  ...
  implicit val functorDecodeRow: Functor[DecodeRow] = new Functor[DecodeRow] {
    override def map[A, B](fa: DecodeRow[A])(f: A => B): DecodeRow[B] = fa.map(f)
  }
